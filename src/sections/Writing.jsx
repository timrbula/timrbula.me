...
const Background = () => (
  <div>
    {/* <Triangle color="backgroundDark" height={['15vh', '10vh']} width={['100vw', '100vw']} invertX /> */}
    {/* <Triangle color="secondary" height={['50vh', '40vh']} width={['70vw', '40vw']} invertY /> */}
    <Triangle color="primaryDark" height={['40vh', '15vh']} width={['100vw', '100vw']} invertX invertY />
  </div>
);
...
function Post({ title, text, image, url, date, time }) {
  const timestamp = `${date} - ${Math.ceil(time)} min`;
  return (
    <Card onClick={() => window.open(url, '_blank')} css={{ cursor: 'pointer' }} p={0}>
...
const parsePost = postFromGraphql => {
  ...
  const image = virtuals.previewImage.imageId && `${MEDIUM_CDN}/${virtuals.previewImage.imageId}`;
  return {
    id,
    title,
